...
mongoose
  .connect("mongodb://usuario:password@bd/pruebas?authSource=admin")
  .then(() => {
...
const productoSerializer = z.object({
  nombre: z.string(),
  precio: z.number().max(100).min(0),
});

servidor
  .route("/productos")
  .post(async (req, res) => {
    // el safeParse si todo esta bien el success sera true caso contrario sera falso y tendremos los errores en la variables errors
    const serializador = productoSerializer.safeParse(req.body);

    if (serializador.success) {
      const data = req.body;
      const nuevoProducto = new ProductoModel({
        nombre: data.nombre,
        precio: data.precio,
      }); // new ProductoModel(data)
      // OTRA FORMA DE CREAR
